...
function App() {
  ...
  return (
    <div className="h-screen bg-bg-dark text-text-primary transition-colors duration-300 font-sans flex flex-col overflow-hidden">
      {/* Header */}
      <header className="glass-panel mx-2 mt-2 p-3 md:mx-6 md:mt-6 md:p-4 flex flex-wrap md:flex-nowrap justify-between items-center shrink-0 z-50 gap-2">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setCurrentView('home')}>
          <div className="w-8 h-8 md:w-10 md:h-10 bg-accent-primary/20 rounded-2xl flex items-center justify-center shadow-lg overflow-hidden shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className="w-5 h-5 md:w-6 md:h-6 text-accent-primary fill-current">
          ...
          <div>
            <h1 className="text-lg md:text-xl font-bold tracking-tight whitespace-nowrap">TOKYO STICKER DB</h1>
            <p className="text-[10px] md:text-xs text-text-secondary uppercase tracking-widest hidden md:block">A work in progress</p>
          </div>
        </div>

        <div className="flex items-center gap-2 md:gap-4 ml-auto overflow-x-auto no-scrollbar max-w-full">
          {/* Navigation actions */}
          <button
            onClick={() => setCurrentView('global')}
            className={`text-xs md:text-sm font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-full transition-colors whitespace-nowrap ${currentView === 'global' ? 'bg-white text-black' : 'hover:bg-white/10'}`}
          >
            ALL STICKERS
          </button>
          <button
            onClick={() => setCurrentView('grouped')}
            className={`text-xs md:text-sm font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-full transition-colors whitespace-nowrap ${currentView === 'grouped' ? 'bg-white text-black' : 'hover:bg-white/10'}`}
          >
            GROUPED
          </button>
          ...
          {/* Clustering Progress - hide in static mode or on very small screens if needed */}
          {!STATIC_MODE && clusterLoading && (
            <div className="hidden md:flex flex-col w-40 gap-1 mr-2 px-3 py-1 bg-bg-dark/30 rounded-lg border border-white/5">
          ...
          {/* System status - hide in static mode and mobile */}
          {!STATIC_MODE && (
            <div className="hidden md:flex items-center gap-2 text-sm font-medium px-3 py-1 bg-bg-dark/50 rounded-full border border-white/5">
      ...
      {/* Main Content Router */}
      <main className="flex-1 flex flex-col gap-6 p-2 md:p-6 min-h-0 overflow-hidden">
        {currentView === 'global' ? (
          <GlobalGallery />
        ) : currentView === 'grouped' ? (
          <GroupedGallery />
